...
import { Sparkles, Send, Loader2, Check, Plus, Edit2, Trash2, AlertCircle, X } from 'lucide-react'
...
export default function AiChatWidget({ onNavigateToSettings }: AiChatWidgetProps) {
  ...
  return (
    <div className="fixed bottom-16 right-4 sm:bottom-6 sm:right-6 z-50 flex flex-col items-end gap-3">
      {/* Chat panel */}
      {isOpen && (
        <div
          className="w-[min(calc(100vw-2rem),24rem)] flex flex-col bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden"
          style={{ maxHeight: 'calc(100dvh - 10rem)' }}
        >
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-200 dark:border-gray-700 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300 shrink-0">
          ...
          {aiSettings.provider === 'gemini' && !apiKey && (
            <div className="mx-4 mt-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-sm text-amber-800 dark:text-amber-300 flex flex-col gap-2 shrink-0">
              <div className="flex items-start gap-2">
                <AlertCircle size={14} className="shrink-0 mt-0.5" />
                <span>Gemini API key not set. Please add it in Settings.</span>
              </div>
              <button
                onClick={() => { onNavigateToSettings(); setIsOpen(false) }}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm ${msg.role === 'user'
                    ...
                    : msg.role === 'system'
                      ? 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 text-xs text-center mx-auto'
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded-2xl rounded-bl-none px-3 py-2 flex items-center gap-2 text-sm">
            ...
            <textarea
              ...
              placeholder={
                aiSettings.provider === 'local'
                  ? aiSettings.localEndpoint ? 'タスクの追加・編集・削除を依頼 (Ctrl+Enter)' : 'Settings でローカルLLMを設定してください'
                  : apiKey ? 'タスクの追加・編集・削除を依頼 (Ctrl+Enter)' : 'Settings で Gemini API キーを設定してください'
              }
              ...
              disabled={!canSubmit}
            />
            <Button type="submit" disabled={!input.trim() || !canSubmit} className="shrink-0 px-3 h-10 mb-[1px]">
              <Send size={16} />
            </Button>
          </form>
        </div>
      ...
      <button
        ...
        className="w-11 h-11 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg flex items-center justify-center transition-colors"
